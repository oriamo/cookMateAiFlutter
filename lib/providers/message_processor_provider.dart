...
class EnhancedChatNotifier extends StateNotifier<List<ChatMessage>> {
  final Ref _ref;
  final StateNotifierProvider<EnhancedChatNotifier, List<ChatMessage>>
      _originalProvider;

  EnhancedChatNotifier(
      this._ref, this._originalProvider, List<ChatMessage> initialMessages)
  ...
  Future<void> processAssistantMessage(ChatMessage message) async {
    if (message.role == MessageRole.assistant) {
      ...
      final timerCreated =
          await messageProcessor.processAIMessage(message.content);

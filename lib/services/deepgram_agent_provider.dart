...
final deepgramAgentProvider =
    ChangeNotifierProvider<DeepgramAgentProvider>((ref) {
...
class DeepgramAgentProvider extends ChangeNotifier {
  ...
  double _noiseTolerance =
      25.0; // Default to slightly higher than service default
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      if (isInitialized) {
        ...
        _deepgramAgentService
            .setContinuousListening(_continuousListeningEnabled);
        ...
        // Add welcome message
        _addSystemMessage(
            'Welcome to live voice conversation mode powered by Deepgram. You can speak or type to interact with the AI.');
        _addSystemMessage(
            'Use the settings menu to adjust noise tolerance for your environment.');
      } else {
  ...
  void _setupListeners() {
    ...
    _deepgramAgentService.onMessage.listen((message) {
      ...
      if (message.contains("Hey there") ||
        ...
        debugPrint(
            '🟢 DEEPGRAM PROVIDER: Adding explicit user message: $message');
      ...
      if (message.contains("I can definitely assist") ||
        ...
        debugPrint(
            '🟢 DEEPGRAM PROVIDER: Adding explicit agent response: $message');
      ...
      if (_deepgramAgentService.state == DeepgramAgentState.listening) {
        debugPrint(
            '🟢 DEEPGRAM PROVIDER: Adding user transcription from listening state: $message');
        _addUserMessage(message);
        return;
      }
      ...
      if (_deepgramAgentService.state == DeepgramAgentState.speaking) {
        debugPrint(
            '🟢 DEEPGRAM PROVIDER: Adding agent message from speaking state: $message');
        _addAgentMessage(message);
        return;
      }
      ...
      if (_messages.isNotEmpty &&
          _messages.last.type == DeepgramAgentMessageType.user) {
        debugPrint(
            '🟢 DEEPGRAM PROVIDER: Adding agent response based on last message: $message');
        _addAgentMessage(message);
        return;
      }
      ...
      debugPrint(
          '🟡 DEEPGRAM PROVIDER: Adding message with unknown role: $message');
      _addAgentMessage(message);
    });
  ...
  Future<void> _processMessageForTimers(String content) async {
    try {
      ...
      if (content.toLowerCase().contains('set up a timer for') ||
        ...
        if (timerDetected) {
          debugPrint(
              '🕒 DEEPGRAM PROVIDER: Timer successfully created from message');

          // Add a confirmation system message
          _addSystemMessage(
              'Timer created successfully! You can see active timers below.');
        } else {
          debugPrint(
              '🕒 DEEPGRAM PROVIDER: Timer format detected but timer creation failed');
        }
      }
    } catch (e) {
      debugPrint(
  ...
  void setContinuousListening(bool enabled) {
    ...
    if (enabled) {
      _addSystemMessage(
          'Continuous listening mode enabled. The AI will listen for new queries after responding.');
    } else {
      _addSystemMessage(
          'Continuous listening mode disabled. Tap the mic to start each new query.');
    }
  ...
  void setDisableInterruptions(bool disable) {
    ...
    if (disable) {
      _addSystemMessage(
          'Interruptions disabled. You cannot interrupt the AI while it is speaking.');
    } else {
      _addSystemMessage(
          'Interruptions enabled. You can speak to interrupt the AI while it is speaking.');
    }
  ...
  void setNoiseTolerance(double tolerance) {
    ...
    _addSystemMessage(
        'Noise tolerance set to ${tolerance.toStringAsFixed(1)}. Higher values ignore more background noise.');
  ...
  Future<void> toggleSpeakerphone() async {
    ...
    try {
      final success =
          await _deepgramAgentService.setSpeakerphoneEnabled(newValue);
      if (success) {

...
class DeepgramAgentService {
  ...
  final int _heartbeatIntervalSeconds =
      2; // Send heartbeat every 2 seconds to prevent timeout
  final int _forcedAudioIntervalMs =
      250; // Send audio data every 250ms regardless of state
  ...
  bool _disableInterruptions =
      false; // When true, user speech doesn't interrupt AI
  double _noiseTolerance = 20.0; // Default threshold for detecting speech
  double _interruptionThreshold =
      35.0; // Higher threshold for interrupting during AI speech
  ...
  static const MethodChannel _audioChannel =
      MethodChannel('com.oraimo.us.cook_mate_ai/audio_stream');
  ...
  Future<bool> initialize() async {
    ...
    try {
      ...
      if (_apiKey == null || _apiKey!.isEmpty) {
        _errorController
            .add('Deepgram API key not found. Please add it to .env file.');
        return false;
      }
    ...
    try {
      ...
      final result = await _audioChannel.invokeMethod<bool>('initAudioStream', {
        ...
        'enableCommunicationMode':
            _continuousListeningEnabled, // Enable phone call mode for continuous listening
      });

      _isAudioStreamInitialized = result ?? false;
      debugPrint(
          '🔊 DEEPGRAM: Native audio stream initialized: $_isAudioStreamInitialized');
  ...
  Future<bool> _setCommunicationMode(bool enabled) async {
    try {
      debugPrint(
          '🔊 DEEPGRAM: ${enabled ? "Enabling" : "Disabling"} communication mode');
      final result =
          await _audioChannel.invokeMethod<bool>('enableCommunicationMode', {
        'enabled': enabled,
      });
      return result ?? false;
    } catch (e) {
  ...
  Future<bool> setSpeakerphoneEnabled(bool enabled) async {
    try {
      ...
      final result = await _audioChannel
          .invokeMethod<bool>('setSpeakerphoneOn', {'enabled': enabled});
      return result ?? false;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _getAudioStats() async {
    ...
    try {
      final result = await _audioChannel
          .invokeMethod<Map<dynamic, dynamic>>('getAudioStats');
    ...
    try {
      ...
      if (_isAudioStreamInitialized) {
        ...
        try {
          ...
          debugPrint(
              '🔴 DEEPGRAM: Error sending audio data to native player: $e');

          // If native playback fails, initialize it again
          if (!await _initAudioStream()) {
            // If re-init fails, fall back to TTS
            debugPrint(
                '🔴 DEEPGRAM: Failed to reinitialize native audio - falling back to TTS');
            await _tts.speak(
                'Audio playback failed. Falling back to text-to-speech.');
          }
        }
      } else {
        // If native audio isn't available, try to initialize it
        debugPrint(
            '🔊 DEEPGRAM: Native audio not initialized - attempting to initialize');
        if (await _initAudioStream()) {
          ...
          // Fall back to TTS
          debugPrint(
              '🔴 DEEPGRAM: Native audio initialization failed - falling back to TTS');
          await _tts.speak(
              'Audio playback not available. Using text-to-speech instead.');
        }
      }
    } catch (e) {
  ...
  Future<bool> connect() async {
    ...
    debugPrint(
        '🔵 DEEPGRAM: Attempting to connect. API key exists: ${_apiKey != null && _apiKey!.isNotEmpty}');
    debugPrint(
        '🔵 DEEPGRAM: API key first 4 chars: ${_apiKey != null ? _apiKey!.substring(0, math.min(4, _apiKey!.length)) + "..." : "null"}');
    ...
    try {
      ...
      try {
        ...
        debugPrint(
            '🟢 DEEPGRAM: Connection completed successfully, state updated to connected');
        return true;
      } catch (socketError) {
    ...
    _channel!.stream.listen(
      (dynamic message) {
        try {
          ...
          debugPrint(
              '🟢 DEEPGRAM: Received message: ${message.toString().substring(0, math.min(100, message.toString().length))}...');
            ...
            switch (data['type']) {
              case 'ConversationText':
                ...
                if (data.containsKey('content')) {
                  ...
                  if (transcript.isNotEmpty) {
                    debugPrint(
                        '🟢 DEEPGRAM: Transcript: $transcript, role: ${data['role'] ?? 'unknown'}');
              ...
              case 'UserStartedSpeaking':
                ...
                if (_continuousListeningEnabled && !_disableInterruptions) {
                  debugPrint(
                      '🔊 DEEPGRAM: User interrupted AI, stopping audio playback');
                  _stopAudioStream();
                } else {
              ...
              case 'AgentStoppedSpeaking':
                ...
                debugPrint(
                    '🔊 DEEPGRAM: Agent finished speaking, letting audio complete');
                ...
                Future.delayed(Duration(seconds: 1), () async {
                  ...
                  if (_state == DeepgramAgentState.speaking) {
                    if (_continuousListeningEnabled) {
                      ...
                      debugPrint(
                          '🔊 DEEPGRAM: Continuous listening active, returning to listening state');
                      _updateState(DeepgramAgentState.listening);
              ...
              case 'EndOfThought':
                ...
                debugPrint(
                    '🔊 DEEPGRAM: Received EndOfThought - AI finished processing');
              ...
              case 'AgentAudioDone':
                ...
                debugPrint(
                    '🔊 DEEPGRAM: Received AgentAudioDone - Audio streaming complete');
              ...
              default:
                debugPrint(
                    '🟡 DEEPGRAM: Unknown message type: ${data['type']}');
                break;
            }
          } else {
            debugPrint(
                '🟡 DEEPGRAM: Message doesn\'t contain type field: $data');
          }
        } catch (e) {
  ...
  void _handleSpeechRecognition(String transcript, bool isFinal) {
    debugPrint(
        '🟢 DEEPGRAM: Handling speech transcript: "$transcript", isFinal: $isFinal');
    ...
    if (isFinal) {
      ...
      debugPrint(
          '🟢 DEEPGRAM: Adding final transcript to message stream: "$transcript"');
      _messageController.add(transcript);
      _updateState(DeepgramAgentState.processing);
      ...
      Future.delayed(Duration(milliseconds: 500), () {
        ...
        if (_state == DeepgramAgentState.processing) {
          debugPrint(
              '🔵 DEEPGRAM: Auto-transitioning back to listening state to maintain connection');
          _updateState(DeepgramAgentState.listening);
        }
      });
    } else {
  ...
  Future<bool> startListening() async {
    debugPrint(
        '🎤 DEEPGRAM: Starting listening mode. Connected: $_isConnected, Already listening: $_isListening');
    ...
    if (!_isConnected) {
      ...
      try {
        connected =
            await connect().timeout(Duration(seconds: 5), onTimeout: () {
          debugPrint(
              '🔴 DEEPGRAM: Connection attempt timed out after 5 seconds');
          return false;
        });
      } catch (e) {
      ...
      if (!connected) {
        ...
        _errorController.add(
            'Failed to connect to Deepgram voice service. Please check your internet connection and try again.');
        return false;
      }
    ...
    if (_isListening) {
      debugPrint(
          '🎤 DEEPGRAM: Already listening, ensuring timers and audio stream are active');
      ...
      if (_recordingSubscription == null || _recorder == null) {
        debugPrint(
            '🔴 DEEPGRAM: Audio stream not active despite being in listening state - restarting audio');
        await _streamAudio();
      }
    ...
    try {
      ...
      if (!_continuousListeningEnabled) {
        debugPrint(
            '🎤 DEEPGRAM: Enabling continuous listening mode for more stable connections');
      ...
      debugPrint(
          '🟢 DEEPGRAM: Status: Connected: $_isConnected, Listening: $_isListening, Continuous: $_continuousListeningEnabled, State: $_state');
      ...
      if (_isConnected) {
        debugPrint(
            '🎤 DEEPGRAM: Will retry starting audio stream in 1 second after failure');
  ...
  Future<bool> stopListening() async {
    ...
    try {
      ...
      if (!_continuousListeningEnabled) {
        ...
        debugPrint(
            '🔊 DEEPGRAM: Keeping audio stream active (continuous mode)');
  ...
  Future<void> _streamAudio() async {
    ...
    try {
      ...
      final config = RecordConfig(
        ...
        autoGain: _continuousListeningEnabled
            ? true
            : false, // Auto gain helps with distant speech
        echoCancel:
            _continuousListeningEnabled, // Echo cancellation for continuous mode
        noiseSuppress: true, // Always use noise suppression
      );

      debugPrint(
          '🟢 DEEPGRAM: Starting audio stream with PCM 16-bit, 16kHz, mono, continuous mode: $_continuousListeningEnabled');
      ...
      _recordingSubscription = stream.listen(
        (data) {
          if (_isConnected && data.isNotEmpty) {
            ...
            if (!hasSentAudioData) {
              debugPrint(
                  '🟢 DEEPGRAM: First audio packet received and sent: ${data.length} bytes');
            ...
            if (packetCount % 20 == 0) {
              ...
              debugPrint(
                  '🎤 DEEPGRAM: Audio stream stats: $packetCount packets, ${totalBytesSent ~/ 1024} KB total, '
            ...
            try {
              if (_channel != null) {
                ...
                if (_continuousListeningEnabled &&
                    _state == DeepgramAgentState.speaking &&
                    !_disableInterruptions) {
                  final hasSignificantAudio = _detectSignificantAudio(data);
                  if (hasSignificantAudio) {
                    debugPrint(
                        '🔊 DEEPGRAM: Detected user speech during AI response - possible interruption');
                    _stopAudioStream();
                    _updateState(DeepgramAgentState.listening);
                  }
                }
                ...
                debugPrint(
                    '🔴 DEEPGRAM: WebSocket channel is null while trying to send audio packet #$packetCount');
              ...
              debugPrint(
                  '🔴 DEEPGRAM: Error sending audio packet #$packetCount: $e');
            ...
            debugPrint(
                '🔴 DEEPGRAM: Not connected while receiving audio packet #$packetCount');
        ...
        onError: (e) {
          ...
          if (_continuousListeningEnabled && _isConnected) {
            debugPrint(
                '🔵 DEEPGRAM: Attempting to restart audio stream after error');
        ...
        onDone: () {
          debugPrint(
              '🔵 DEEPGRAM: Audio stream done, packets sent: $packetCount, total: ${totalBytesSent ~/ 1024} KB');

          // If continuous listening is enabled and we're still connected,
          // but the stream ended, restart it immediately
          if (_continuousListeningEnabled && _isConnected) {
            debugPrint(
                '🔵 DEEPGRAM: Restarting audio stream for continuous listening - previous stream ended');
            _streamAudio();
          }
        },
      );
      ...
      if (_continuousListeningEnabled && _isConnected) {
        debugPrint(
            '🔵 DEEPGRAM: Will attempt to restart audio stream in 2 seconds after failure');
    ...
    Timer.periodic(const Duration(milliseconds: 300), (timer) async {
      ...
      try {
        ...
        if (math.Random().nextInt(5) == 0) {
          // Log only ~20% of readings
          debugPrint(
              '🔵 DEEPGRAM: Audio amplitude: ${amplitude.current.toStringAsFixed(1)}, state: $_state');
        }

        // IMPORTANT: Handle amplitude-based state transitions
        if (_continuousListeningEnabled &&
            amplitude.current > _noiseTolerance) {
          if (_state == DeepgramAgentState.connected ||
              _state == DeepgramAgentState.idle) {
            // Wake from idle/connected state if we detect speech
            debugPrint(
                '🔊 DEEPGRAM: Detected speech in continuous mode (${amplitude.current.toStringAsFixed(1)}), activating listening');
            _updateState(DeepgramAgentState.listening);
            _resetInactivityTimer();
            ...
            debugPrint(
                '🔊 DEEPGRAM: Detected user interruption during AI speech (${amplitude.current.toStringAsFixed(1)}), stopping playback');
            _stopAudioStream();
            _updateState(DeepgramAgentState.listening);
            _resetInactivityTimer();
          }
  ...
  Future<void> processImageWithLlm(String userMessage) async {
    ...
    try {
      ...
      final response = await _llmService
          .generateMultimodalResponse(userMessage, [imageData]);
  ...
  void _startInactivityTimer() {
    ...
    _inactivityTimer = Timer(Duration(seconds: _inactivityTimeoutSeconds), () {
      debugPrint(
          '🔵 DEEPGRAM: Inactivity timeout reached, closing connection to save costs');

      // If we're still connected but idle, disconnect to save costs
      if (_isConnected &&
          (_state == DeepgramAgentState.connected ||
              _state == DeepgramAgentState.idle)) {
  ...
  void _startHeartbeatTimer() {
    ...
    _heartbeatTimer =
        Timer.periodic(Duration(seconds: _heartbeatIntervalSeconds), (timer) {
      if (!_isConnected) {
        _stopHeartbeatTimer();
        return;
      }
  ...
  void _startForcedAudioTimer() {
    ...
    debugPrint(
        '🔵 DEEPGRAM: Starting forced audio timer (${_forcedAudioIntervalMs}ms intervals)');

    _forcedAudioTimer =
        Timer.periodic(Duration(milliseconds: _forcedAudioIntervalMs), (_) {
      if (!_isConnected || _channel == null) {
        debugPrint(
            '🔴 DEEPGRAM: Not connected in forced audio timer - stopping timer');
        _stopForcedAudioTimer();
        return;
      }
      ...
      try {
        ...
        if (_forcedAudioCount % 10 == 0) {
          debugPrint(
              '🔉 DEEPGRAM: Forced audio packet #$_forcedAudioCount sent (${forcedAudioData.length} bytes, ${timeSinceLastAudio}ms since last)');
        }
        ...
        debugPrint(
            '🔴 DEEPGRAM: Error sending forced audio packet #$_forcedAudioCount: $e');
        ...
        if (_isConnected && (_forcedAudioCount % 5 == 0)) {
          debugPrint(
              '🔴 DEEPGRAM: Multiple forced audio packet failures - attempting reconnection');
          _reconnect();
        }
      }
    });
    ...
    debugPrint(
        '🟢 DEEPGRAM: Forced audio timer started with ${_forcedAudioIntervalMs}ms intervals');
  }
  ...
  void _stopForcedAudioTimer() {
    if (_forcedAudioTimer != null) {
      debugPrint(
          '🔵 DEEPGRAM: Stopping forced audio timer (sent $_forcedAudioCount packets)');
  ...
  void setContinuousListening(bool enabled) {
    ...
    debugPrint(
        '🔵 DEEPGRAM: Continuous listening ${enabled ? 'enabled' : 'disabled'}');
  ...
  void setDisableInterruptions(bool disable) {
    ...
    debugPrint(
        '🔵 DEEPGRAM: AI interruptions ${disable ? 'disabled' : 'enabled'}');
  }
  ...
  void setNoiseTolerance(double tolerance) {
    ...
    debugPrint(
        '🔵 DEEPGRAM: Interruption threshold set to $_interruptionThreshold');
  }
  ...
  Future<void> _reconnect() async {
    ...
    if (_isReconnecting) {
      debugPrint(
          '🔵 DEEPGRAM: Reconnection already in progress, skipping duplicate request');
      return;
    }
    ...
    try {
      ...
      debugPrint(
          '🔵 DEEPGRAM: Attempting to reconnect... (previous state: $previousState, was listening: $wasListening)');
      ...
      try {
        connected =
            await connect().timeout(Duration(seconds: 5), onTimeout: () {
          debugPrint(
              '🔴 DEEPGRAM: Reconnection attempt timed out after 5 seconds');
          return false;
        });
      } catch (e) {
      ...
      if (connected) {
        ...
        if (wasListening) {
          debugPrint(
              '🟢 DEEPGRAM: Restarting listening after successful reconnection');
          await startListening();
        } else {
          debugPrint(
              '🟢 DEEPGRAM: Connection restored but not resuming listening (wasn\'t active before)');
        }
      } else {
        ...
        if (wasListening) {
          _errorController
              .add('Lost connection to voice service. Please try again.');
          _updateState(DeepgramAgentState.idle);
        }
        ...
        if (wasListening && previousState != DeepgramAgentState.idle) {
          debugPrint(
              '🔵 DEEPGRAM: Will attempt one more reconnection in 3 seconds');
